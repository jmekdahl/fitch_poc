...
}}}h=e.module("ngRoute",["ng"]).provider("$route",function(){function h(a,c){return e.extend(new(e.extend(function(){},{prototype:a})),c)}function q(a,e){var b=e.caseInsensitiveMatch,f={originalPath:a,regexp:a},h=f.keys=[];return a=a.replace(/([().])/g,"\\$1").replace(/(\/)?:(\w+)([\?|\*])?/g,function(a,e,b,c){return a="?"===c?c:null,c="*"===c?c:null,h.push({name:b,optional:!!a}),e=e||"",""+(a?"":e)+"(?:"+(a?e:"")+(c&&"(.+?)"||"([^/]+)")+(a||"")+")"+(a||"")}).replace(/([\/$\*])/g,"\\$1"),f.regexp=RegExp("^"+a+"$",b?"i":""),f}var k={};this.when=function(a,c){if(k[a]=e.extend({reloadOnSearch:!0},c,a&&q(a,c)),a){var b="/"==a[a.length-1]?a.substr(0,a.length-1):a+"/";k[b]=e.extend({redirectTo:a},q(b,c))}return this},this.otherwise=function(a){return this.when(null,a),this},this.$get=["$rootScope","$location","$routeParams","$q","$injector","$http","$templateCache","$sce",function(a,c,b,f,n,q,v,l){function g(){var d=t(),m=r.current;d&&m&&d.$$route===m.$$route&&e.equals(d.pathParams,m.pathParams)&&!d.reloadOnSearch&&!x?(m.params=d.params,e.copy(m.params,b),a.$broadcast("$routeUpdate",m)):(d||m)&&(x=!1,a.$broadcast("$routeChangeStart",d,m),(r.current=d)&&d.redirectTo&&(e.isString(d.redirectTo)?c.path(u(d.redirectTo,d.params)).search(d.params).replace():c.url(d.redirectTo(d.pathParams,c.path(),c.search())).replace()),f.when(d).then(function(){if(d){var c,b,a=e.extend({},d.resolve);return e.forEach(a,function(d,c){a[c]=e.isString(d)?n.get(d):n.invoke(d)}),e.isDefined(c=d.template)?e.isFunction(c)&&(c=c(d.params)):e.isDefined(b=d.templateUrl)&&(e.isFunction(b)&&(b=b(d.params)),b=l.getTrustedResourceUrl(b),e.isDefined(b)&&(d.loadedTemplateUrl=b,c=q.get(b,{cache:v}).then(function(a){return a.data}))),e.isDefined(c)&&(a.$template=c),f.all(a)}}).then(function(c){d==r.current&&(d&&(d.locals=c,e.copy(d.params,b)),a.$broadcast("$routeChangeSuccess",d,m))},function(c){d==r.current&&a.$broadcast("$routeChangeError",d,m,c)}))}function t(){var a,b;return e.forEach(k,function(f){var p;if(p=!b){var s=c.path();p=f.keys;var l={};if(f.regexp)if(s=f.regexp.exec(s)){for(var g=1,q=s.length;q>g;++g){var n=p[g-1],r="string"==typeof s[g]?decodeURIComponent(s[g]):s[g];n&&r&&(l[n.name]=r)}p=l}else p=null;else p=null;p=a=p}p&&(b=h(f,{params:e.extend({},c.search(),a),pathParams:a}),b.$$route=f)}),b||k[null]&&h(k[null],{params:{},pathParams:{}})}function u(a,c){var b=[];return e.forEach((a||"").split(":"),function(a,d){if(0===d)b.push(a);else{var e=a.match(/(\w+)(.*)/),f=e[1];b.push(c[f]),b.push(e[2]||""),delete c[f]}}),b.join("")}var x=!1,r={routes:k,reload:function(){x=!0,a.$evalAsync(g)}};return a.$on("$locationChangeSuccess",g),r}]}),h.provider("$routeParams",function(){this.$get=function(){return{}}}),h.directive("ngView",u),h.directive("ngView",z),u.$inject=["$route","$anchorScroll","$animate"],z.$inject=["$compile","$controller","$route"]}(window,window.angular);var controllers={dealsListController:function($scope,$location,filterFilter,sharedService){$scope.sharedData=sharedService.sharedObject,$scope.predicate=$scope.sharedData.defaultPredicate,$scope.deals=$scope.sharedData.deals,$scope.selection=$scope.sharedData.selection.group,!$scope.deals.length||$scope.selection.length||$scope.deals[0].selected||angular.forEach($scope.deals,function(value){value.selected=!1}),$scope.selectedDeals=function(){return filterFilter($scope.deals,{selected:!0})},$scope.$watch("deals|filter:{selected:true}",function(nv){$scope.selection=nv.map(function(deal){return{id:deal.id,name:deal.name}})},!0),$scope.setSelection=function(){!$scope.selection.length||$scope.selection.length<sharedService.sharedObject.selection.min?alert("Select at least two deals to compare."):$scope.selection.length>sharedService.sharedObject.selection.max?alert("You can select a maximum of "+sharedService.sharedObject.selection.max):(sharedService.sharedObject.selection.group=$scope.selection,$location.path(sharedService.sharedObject.comparison.dflt))}},compareController:function($scope,$q,$location,sharedService,compare){if($scope.sharedData=sharedService.sharedObject,$scope.sharedData.selection.group.length||$location.path("/list"),$scope.selectedDeals=[],angular.forEach($scope.sharedData.deals,function(entValue){angular.forEach($scope.sharedData.selection.group,function(selValue){selValue.id===entValue.id&&$scope.selectedDeals.push(entValue)})}),$scope.displayData=[],$scope.sharedData.dataMap[compare].length)angular.forEach($scope.sharedData.dataMap[compare],function(mapValue,mapKey){var row={name:$scope.sharedData.dataMap[compare][mapKey],cols:[]};angular.forEach($scope.selectedDeals,function(dealValue){row.cols.push(dealValue[compare][mapKey])}),$scope.displayData.push(row)});else{var longest=0;angular.forEach($scope.selectedDeals,function(value){longest=value[compare].length>longest?value[compare].length:longest});for(var i=0;longest>i;i++){var row={cols:[]};angular.forEach($scope.selectedDeals,function(dealValue){row.cols.push(dealValue[compare][i])}),$scope.displayData.push(row)}}},compareChartController:function($scope,$location,sharedService){$scope.sharedData=sharedService.sharedObject,$scope.sharedData.selection.group.length||$location.path("/list"),$scope.selectedDeals=[],angular.forEach($scope.sharedData.deals,function(dealValue){angular.forEach($scope.sharedData.selection.group,function(selValue){selValue.id===dealValue.id&&$scope.selectedDeals.push(dealValue)})}),$scope.displayData=[],angular.forEach($scope.sharedData.dataMap.chart.names,function(mapValue,mapKey){var row={name:$scope.sharedData.dataMap.chart.names[mapKey],cols:[]};angular.forEach($scope.selectedDeals,function(dealValue){row.cols.push(dealValue.chart[mapKey])}),$scope.displayData.push(row)}),$scope.calcData={titles:["Average","Min","Max"]},angular.forEach($scope.sharedData.dataMap.chart.calcs,function(mapValue,mapKey){if(mapValue){for(var average=0,tempArray=[],colLength=$scope.displayData[mapKey].cols.length,i=0;colLength>i;i++)average+=parseFloat($scope.displayData[mapKey].cols[i])/colLength||0;tempArray.push(parseFloat(average).toFixed(2)),tempArray.push(parseFloat(.9123*average).toFixed(2)||0),tempArray.push(parseFloat(1.175*average).toFixed(2)||0)}else tempArray=[0,0,0];angular.forEach(tempArray,function(value){$scope.displayData[mapKey].cols.push(value)})})}},cloApp=angular.module("cloApp",["ngRoute"]);cloApp.factory("sharedService",function(){return{sharedObject:{selection:{group:[],min:2,max:5},deals:dealsObj,dataMap:mapObj,defaultPredicate:"name",comparison:{dflt:"/compare_chart"}}}}),cloApp.config(function($routeProvider){$routeProvider.when("/list",{controller:"dealsListController",templateUrl:"tmpl/partials/dealsListView.html"}).when("/compare_glossary",{controller:"compareController",templateUrl:"tmpl/partials/compareView.html",resolve:{compare:function(){return"glossary"}}}).when("/compare_priority",{controller:"compareController",templateUrl:"tmpl/partials/compareBasicView.html",resolve:{compare:function(){return"priority"}}}).when("/compare_replacements",{controller:"compareController",templateUrl:"tmpl/partials/compareView.html",resolve:{compare:function(){return"replacements"}}}).when("/compare_chart",{controller:"compareChartController",templateUrl:"tmpl/partials/compareView.html"}).otherwise({redirectTo:"/list"})}),cloApp.controller(controllers);